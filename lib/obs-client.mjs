...
// Build the GuardDecision envelope from a hook `input` (pure apart from the
// source-app lookup). Exposed for unit tests; emitGuardDecision POSTs the result.
//
// `tool_use_id` goes in the PAYLOAD, not the top-level envelope field. The
// collector's events table carries a UNIQUE(tool_use_id, hook_event_type) dedup
// index (INSERT OR IGNORE), so a top-level id would make a second guard's
// decision on the SAME call collide and get silently dropped — undercounting the
// Guards rollup. In the payload it stays out of that index, and the collector
// reads it there to correlate the deny with its orphaned Pre (exact match instead
// of a ±3s time window).
export function buildGuardEnvelope(input, { guard, rule, decision, reason }) {
  const tool_name = typeof input?.tool_name === "string" ? input.tool_name : undefined;
  const tool_use_id = typeof input?.tool_use_id === "string" ? input.tool_use_id : undefined;
  ...
  if (tool_use_id) payload.tool_use_id = tool_use_id; // orphan↔deny correlation key
...
export function emitGuardDecision(input, { guard, rule, decision, reason, timeoutMs = 2000 }) {
  try {
    return postEnvelope(buildGuardEnvelope(input, { guard, rule, decision, reason }), { timeoutMs });
  } catch {

...
description: >-
  ...
  사람 검토 후 승격까지의 신뢰도 라이프사이클(scaffold → 미확인) → confirmed)을
...
| inferred | `미확인) <설명> [근거: a.java:12]` | 미확인 | 코드 추론, **저신뢰**, 근거 동반, 재생성 가능, 주입 시 "미확인)"가 곧 헤지 |
| confirmed | `<설명> [근거: a.java:12]` | 채택됨 | 사람 검토, **고신뢰**, 동결(덮어쓰지 않음) |

> #115 이전엔 inferred 마커가 `추정)`였다. `apply.mjs`는 둘 다 인식하고 새로 쓸 땐
> `미확인)`을 쓴다. 기존 문서는 `cli.mjs migrate --doc <path> --write`로 일괄 교체한다.
...
- 티어 접두어는 CLI가 붙인다 — 생산자가 `미확인)`을 직접 쓰지 않는다.
...
승인되면 `--write`. 기존 `미확인)` 값은 기본적으로 최신 추론으로 갱신되며,
`--keep-inferred`를 주면 남긴다.

### 4. 사람 검토 → 승격

사용자가 `미확인)` 슬롯을 근거와 대조해 맞다고 판단하면 confirmed로 올린다.
...
승격은 `미확인)` 접두어만 떼고 근거는 남긴다(confirmed 사실의 출처로).

...
node core/observability/server.mjs representative-queries  # #114: observed DbQuery (run_query, successful only) → per-table 대표 쿼리 proposals for a db-schema document's `## 대표 쿼리` slot (format owner: akg; read-only, proposal-only; --window 30d / --alias / --table / --per-table / --min-count / --all-tools / --include-errors / --json)
```
...
| GET | `/stats/turns` | Turn Inspector (#73, `docs/agent-dashboard-turn-inspector-design.md`): one session's events grouped into **turns** (`UserPromptSubmit` → last `Stop`; arrival-race repair, queued-prompt merge only when an in-flight Pre pairs after the prompt, post-stop tail kept but excluded from timing). `session_id` required, `limit` = latest N (≤500, no window — retention bounds it). Per turn: tool/wait/gap time split (main-lane interval **union**; permission-wait carved out of the enclosing call; capped), calls/errors/orphans/`guard_denies` (orphans' top cause is a hook-guard deny — correlated to its Pre by the `tool_use_id` the guard now stamps into the GuardDecision payload, with a ±3s time-window fallback for legacy rows; `guard_denies` counts only denies that orphaned a call), subagent lane, and inefficiency `flags` (`dup-call`/`re-read`/`retry-loop`/`search-storm`/`long-tail`/`gap-heavy`/`orphaned`/`mega-turn` — thresholds via `{"turns":{…}}`). `&turn=<seq>` adds the call timeline + markers. Each turn carries `cost_usd` (main chain) and `cost_subagent_usd` (#81 — the subagent spend that turn triggered, id-joined via the agents' own tool calls; `null` = zero usage rows attributed, never $0.00; compact API calls aren't in usage → the UI badges PreCompact turns). Attribution is single-bucket per usage row — emitted-id match first, `follows` only when emitted is empty, ts-window fallback — and the response totals `usage_cost_usd` / `unattributed_cost_usd` (rows landing in no turn) keep the sum honest. Reads `payload` for ONE session only (`prompt`/`tool_input`/`message`/guard fields) |
| GET | `/stats/fleet-turns` | #82 stages 2-3: fleet-wide aggregates over the materialised `turns` table (no payload). `window` + `source_app`. Returns `totals` (settled_turns, human_turns, avg_calls_per_turn, dup_call_turn_ratio, gap-heavy/mega/interrupted/orphan turns, total/subagent/unattributed cost, cost_incomplete), `by_flag` (turns + cost per inefficiency flag — an **attention signal, not savings**; a multi-flag turn counts in each so Σ ≠ total), `by_app`, and a time `series`. Efficiency ratios exclude virtual (#0) and harness-`auto` turns; cost totals include everything |
| GET | `/health` | liveness + counters (single-instance probe) |
| GET | `/` + `/app.js` | dependency-free dashboard: Live tail, Sessions (rollup + tokens + cost + avg/peak ctx + mega badge + a human session title — generated title, else first prompt, else hash — and the **Turn Inspector drill-down** (#73 stage 2): context-growth curve & compact what-if, then a `/stats/turns`-backed turn list with flags badges + a `[⚑ flags만]` filter, harness-injected turns labeled `⚙` (raw prompt on hover), per-turn lazy detail = tool/wait/gap stack bar + call timeline (gap/`∥`, input summaries, status colors, dup/bg/→next badges, `└ sub:` lanes, dimmed post-stop tail, ⚠ permission / ⛔ guard-deny / ✂ compact markers, row click → raw event JSON)), Tools (latency/error bars), Tokens (cost anatomy stack + baseline/turn-tax/switch-rewrite cards + daily/by app/by model/by tool + trend), Guards (what the git/bash guards blocked), Nudges (ctx-budget `/compact` boundary nudges + compliance), **insight** (#82 stages 2-3: fleet-wide turn stats over the materialised table — totals cards, by-flag + by-project tables, turns-over-time series), fleet strip with per-session context size, per-screen `(?)` help tooltips for derived metrics (strict CSP, same-origin, inline-SVG charts) |

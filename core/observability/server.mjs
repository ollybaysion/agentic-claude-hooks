...
// ── GET /stats/schema-docs (#90 — enrich review queue + apply/promote history)
// Two independent sources, by design (issue #90 reconciliation):
//   • QUEUE   = a fresh FILE SCAN of the user-layer keyword-docs (the same corpus
//     the viewer lists), parsing each "추정) …[근거: …]" inferred slot. Files are
//     the source of truth, so the queue is real the moment docs exist — no events
//     required. These are the slots awaiting human review/promotion.
//   • HISTORY = SchemaDocApply / SchemaDocPromote events (activity log). Empty
//     until enrich-cli --write runs; it records WHAT changed WHEN, not current
//     state. Deriving the queue from events alone would drift from the files
//     (lost events, manual edits) — so the queue never reads events.
// Read-only and loopback-gated like every other /stats endpoint.

// Pull the inferred (추정) slots out of a doc: one per "추정)" occurrence, with
// its inference text and [근거: f:line; …] evidence. Table rows are split on
// UNESCAPED pipes (formatEvidence writes a literal | as \|), so an evidence ref
// or description containing a pipe doesn't truncate the slot. Count matches
// docTiers().inferred by construction (both count 추정) occurrences).
function parseInferredSlots(text) {
  const slots = [];
  const push = (s) => {
    const at = s.indexOf("추정)");
    if (at < 0) return;
    let body = s.slice(at + "추정)".length).trim();
    const evidence = [];
    const em = /\[근거:\s*([^\]]*)\]/.exec(body);
    if (em) {
      for (const ref of em[1].split(";")) { const r = ref.trim(); if (r) evidence.push(r.slice(0, 120)); }
      body = (body.slice(0, em.index) + body.slice(em.index + em[0].length)).trim();
    }
    slots.push({ text: body.slice(0, 400), evidence: evidence.slice(0, 12) });
  };
  for (const line of String(text).split(/\r?\n/)) {
    if (line.indexOf("추정)") < 0) continue;
    if (/^\s*\|/.test(line)) {
      for (const cell of line.split(/(?<!\\)\|/))
        if (cell.indexOf("추정)") >= 0) push(cell.replace(/\\\|/g, "|"));
    } else push(line);
  }
  return slots;
}
...
function handleStatsSchemaDocs(req, res, u) {
  ...
  // QUEUE — file scan (real current state, independent of events)
  const queue = [];
  let slotsPending = 0;
  try {
    for (const { id, index } of userDocIndexes(docsHomeDir())) {
      for (const e of resolveIndexEntries(index)) {
        let text;
        try { text = fs.readFileSync(e.abs, "utf8"); } catch { continue; }
        const slots = parseInferredSlots(text);
        if (!slots.length) continue;
        slotsPending += slots.length;
        queue.push({ instance: id, doc: e.path, path: e.abs, inferred: slots.length, slots });
      }
    }
  } catch (e) { logSafe("schema-docs queue", e); }
  queue.sort((a, b) => b.inferred - a.inferred);
  // HISTORY — SchemaDocApply/SchemaDocPromote events (activity log)
  const history = [];
  let applies = 0, promotes = 0, promotedSlots = 0;
  if (db) {
    try {
      const since = Date.now() - window_ms;
      const rows = db.impl.prepare(
      ...
      for (const r of rows) {
        if (r.hook_event_type === "SchemaDocApply") {
          ...
          history.push({ ts: r.received_at, type: "apply", app: r.source_app, doc: r.doc || "(unknown)",
            filled: arr(r.filled).length, skipped: arr(r.skipped).length });
        } else {
  ...
  json(res, 200, {
    window_ms,
    totals: { docs_pending: queue.length, slots_pending: slotsPending, applies, promotes, promoted_slots: promotedSlots },
    queue, history,
  });
}
...
#rv-queue-rows tr:hover{background:#161b22}
#rv-detail{margin:8px 0 14px}
#rv-detail h3{margin:10px 0 6px;font-size:13px}
.rv-slot{padding:6px 10px;border:1px solid #1c2230;border-radius:5px;margin:4px 0;background:#0d1119}
.rv-slot .rv-txt{display:block;margin-bottom:4px}
.rv-slot .ev{display:inline-block;font-size:11px;color:#6b7686;background:#161b22;border-radius:3px;padding:1px 6px;margin:2px 4px 0 0;font-family:monospace}
...
<header>
  ...
  <nav>
    ...
    <a href="#review" id="tab-review">review</a>
...
<section id="view-review">
  ...
  <div class="toolbar">기간
    <select id="rv-window"><option>7d</option><option selected>30d</option><option>90d</option></select>
    <span data-help="review"></span>
    <span class="dim">대기열 = 문서 파일 스캔(진실원·즉시) · 이력 = apply/promote 이벤트 · 승격은 CLI(사람), 2단계에서 버튼화</span>
  </div>
  <h2>검토 대기열</h2>
  <table>
    <thead><tr><th>instance</th><th>doc</th><th class="num">추정) 슬롯</th></tr></thead>
    <tbody id="rv-queue-rows"></tbody>
  </table>
  <div id="rv-detail"></div>
  <h2>apply / promote 이력</h2>
  <table>
    <thead><tr><th>time</th><th>type</th><th>doc</th><th>app</th><th class="num">slots</th></tr></thead>
    <tbody id="rv-history-rows"></tbody>
  </table>
</section>
...
const DASHBOARD_JS = `(function(){
  ...
  var HELP={
    ...
    review:"enrich 추정) 슬롯의 검토·승격 현황 (db-schema-enrich #89 + 도메인 스킬)\\n• 검토 대기열 = ~/.claude 문서를 직접 스캔한 추정) 슬롯 (파일이 진실원, 즉시 반영) — 행을 누르면 슬롯별 추론 텍스트 + 근거(파일:라인)\\n• 이력 = apply/promote 이벤트 (enrich-cli --write 실행 시 emit, 활동 로그) · 이벤트 유실·수동 편집과 어긋날 수 있어 대기열은 이벤트가 아니라 파일 기준\\n• 승격 = 사람이 CLI로 (추정)→확정) · 대시보드 버튼화는 2단계(쓰기 표면)에서 결정",
  ...
  var TABS=["live","sessions","tools","tokens","guards","nudges","db","docs","review","insight"];
  ...
  // ── review tab (#90 — enrich review queue + apply/promote history). The queue
  // is a FILE SCAN (real pending 추정) slots, shown immediately); history is the
  // apply/promote event log. Click a queue row to see each slot + its evidence.
  function rvSlots(q){ var box=$("rv-detail"); box.textContent="";
    box.appendChild(el("h3",null,q.doc+" — 추정) 슬롯 "+q.inferred));
    (q.slots||[]).forEach(function(s){ var d=el("div","rv-slot");
      d.appendChild(el("span","rv-txt",s.text||"(빈 슬롯)"));
      (s.evidence||[]).forEach(function(ev){ d.appendChild(el("span","ev",ev)); });
      box.appendChild(d); });
    box.scrollIntoView({block:"nearest"}); }
  function loadReview(){ var w=$("rv-window").value;
    getJson("/stats/schema-docs?window="+w).then(function(d){ var t=d.totals||{};
      var box=$("rv-cards"); box.textContent="";
      box.appendChild(card("docs pending",t.docs_pending||0));
      box.appendChild(card("slots pending",t.slots_pending||0));
      box.appendChild(card("applies",t.applies||0));
      box.appendChild(card("promotes",t.promotes||0));
      var qr=$("rv-queue-rows"); qr.textContent="";
      (d.queue||[]).forEach(function(q){ var tr=document.createElement("tr");
        tr.appendChild(cell(q.instance,"dim"));
        tr.appendChild(cell(q.doc));
        tr.appendChild(cell(q.inferred,"num"));
        tr.style.cursor="pointer"; tr.addEventListener("click",function(){ rvSlots(q); });
        qr.appendChild(tr); });
      if(!(d.queue||[]).length){ var e=document.createElement("tr"),etd=el("td","dim","검토 대기 중인 추정) 슬롯 없음 (모두 승격됐거나 문서 없음)"); etd.colSpan=3; e.appendChild(etd); qr.appendChild(e); $("rv-detail").textContent=""; }
      var hr=$("rv-history-rows"); hr.textContent="";
  ...
  $("rv-window").addEventListener("change",loadReview);

  // 30s refresh of whichever analytics tab is visible
  setInterval(function(){ var h=location.hash.slice(1); if(h==="sessions")loadSessions(); else if(h==="tools")loadTools(); else if(h==="tokens")loadTokens(); else if(h==="guards")loadGuards(); else if(h==="nudges")loadNudges(); else if(h==="db")loadDb(); else if(h==="review")loadReview(); else if(h==="insight")loadFleetTurns(); },30000);

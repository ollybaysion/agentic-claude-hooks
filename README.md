...
| [`db-schema-docs`](skills/db-schema-docs/SKILL.md) | `/claude-hooks:db-schema-docs [db-alias] [table ...]` | ✅ active | Generate/regenerate db-schema keyword-docs from a LIVE Oracle schema via the agent-db-plugin MCP tools (`describe_table`/`list_tables`): structural slots auto-filled from the catalog, meaning slots left as `{{...}}` scaffolds. Regeneration-safe via `dbdoc` auto/manual markers — human-written regions (and per-column descriptions) survive; unmarked hand-authored docs are never overwritten. Dry-run → approve → write. Moved from agent-db-plugin so the doc format lives with its owner; the only cross-repo contract is the `describe_table` JSON shape. |
